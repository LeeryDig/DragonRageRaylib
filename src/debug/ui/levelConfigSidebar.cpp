...
void DrawLevelConfigSidebar(GameWorld& gameWorld, const LevelConfigActions& actions) {
    ...
    DrawUiText("Level", static_cast<int>(x + 14.0f), 44, 20, RAYWHITE);
    ...
    if (gameWorld.debugUi.levelConfigTab == 0) {
        ...
        DrawUiText(configLabel.c_str(), static_cast<int>(x + 14.0f), static_cast<int>(y), 14, GRAY);
        y += 28.0f;

        std::vector<std::string> skyboxes = ListSkyboxPaths();
        int count = static_cast<int>(skyboxes.size());
        if (count == 0) {
            DrawUiText("No skybox files in resources/assets/skybox", static_cast<int>(x + 14.0f), static_cast<int>(y), 14, ORANGE);
            return;
        }
        ...
        for (int i = 0; i < visibleRows; ++i) {
            ...
            DrawUiText(label.c_str(), static_cast<int>(row.x + 8.0f), static_cast<int>(row.y + 10.0f), 14, RAYWHITE);
        ...
        DrawUiText(TextFormat("Current: %s", currentLabel.c_str()), static_cast<int>(x + 14.0f), static_cast<int>(bottomY), 14, LIGHTGRAY);
        return;
    }

    if (gameWorld.debugUi.levelConfigTab == 1) {
        FogConfig& fog = gameWorld.world.runtimeConfig.fog;
        DrawUiText("Simple linear distance fog. Skybox stays clean.", static_cast<int>(x + 14.0f), static_cast<int>(y), 14, GRAY);
        y += 32.0f;
        ...
        DrawUiText(TextFormat("RGB %d %d %d", fog.color.r, fog.color.g, fog.color.b), static_cast<int>(x + 96.0f), static_cast<int>(y + 6.0f), 14, LIGHTGRAY);
        y += 42.0f;
        DrawUiText(TextFormat("Mode: linear  %s", gameWorld.debugUi.configDirty ? "[unsaved]" : "[saved]"), static_cast<int>(x + 14.0f), static_cast<int>(y), 14, gameWorld.debugUi.configDirty ? YELLOW : LIGHTGRAY);
        return;
    }

    if (gameWorld.debugUi.levelConfigTab == 2) {
        Color& ambient = gameWorld.world.runtimeConfig.lighting.ambient;
        DrawUiText("Ambient light per level. Realtime, save writes JSON.", static_cast<int>(x + 14.0f), static_cast<int>(y), 14, GRAY);
        y += 34.0f;
        y += 44.0f;
        ...
        DrawRectangleLinesEx(Rectangle{x + 14.0f, y, 70.0f, 28.0f}, 1.0f, RAYWHITE);
        DrawUiText(TextFormat("RGB %d %d %d", ambient.r, ambient.g, ambient.b), static_cast<int>(x + 96.0f), static_cast<int>(y + 6.0f), 14, LIGHTGRAY);
        y += 46.0f;
        DrawUiText("Shadow config saved only; renderer pass pending.", static_cast<int>(x + 14.0f), static_cast<int>(y), 14, ORANGE);
        y += 26.0f;
    ...
    DrawUiText("Ordem da lista define level inicial no boot.", static_cast<int>(x + 14.0f), static_cast<int>(y), 14, GRAY);
    y += 28.0f;
    float rowH = 46.0f;
    int count = static_cast<int>(gameWorld.world.levelsConfig.levels.size());
    if (count == 0) {
        DrawUiText("No levels in levels.json", static_cast<int>(x + 14.0f), static_cast<int>(y), 16, ORANGE);
        return;
    }
    ...
    for (int i = 0; i < visibleRows; ++i) {
        ...
        DrawUiText(TextFormat("%d. %s%s", levelIndex + 1, levelLabel.c_str(), current ? "  [loaded]" : ""), static_cast<int>(row.x + 8.0f), static_cast<int>(row.y + 6.0f), 16, RAYWHITE);
        DrawUiText(TextFormat("Level file: %s", FileNameFromPath(entry.path).c_str()), static_cast<int>(row.x + 8.0f), static_cast<int>(row.y + 25.0f), 13, LIGHTGRAY);
        if (hovered && IsMouseButtonPressed(MOUSE_LEFT_BUTTON)) {
    ...
    if (selectedEntry != nullptr) {
        ...
        DrawUiText(TextFormat("Selected: %s", selectedLabel.c_str()), static_cast<int>(x + 14.0f), static_cast<int>(actionY), 16, YELLOW);
        actionY += 24.0f;
        std::string selectedConfigLabel = selectedEntry->configPath.empty() ? "(planned)" : FileNameFromPath(selectedEntry->configPath);
        DrawUiText(TextFormat("Config: %s", selectedConfigLabel.c_str()), static_cast<int>(x + 14.0f), static_cast<int>(actionY), 14, GRAY);
        actionY += 34.0f;
    }
